...
export interface FareyPrediction {
  left: [number, number];
  right: [number, number];
  candidates: Array<{ label: string; length: number; count: number; color: string }>;
  valid: boolean;
}
...
export function normalizeScenario(input: Partial<Scenario>): Scenario {
  ...
  const alpha = alphaMode === 'rational'
    ? frac(p / q)
    : frac(Number.isFinite(input.alpha) ? Number(input.alpha) : (Math.sqrt(5) - 1) / 2);
  return {
    alpha,
    alphaMode,
    rationalP: p,
...
function gapsFromSorted(points: OrbitPoint[]): Gap[] {
  ...
  return sorted.map((point, index) => {
    ...
    const length = index === sorted.length - 1 ? 1 - point.value + next.value : next.value - point.value;
    return { id: 'gap-' + index, start: point.value, end: next.value, length, type: '', rank: index };
  });
}
...
function gapGroups(gaps: Gap[]): GapGroup[] {
  ...
  ordered.forEach((gap) => {
    const current = groups.find((group) => Math.abs(group.length - gap.length) < 1e-8);
    if (current) current.count += 1;
    else {
      const index = groups.length;
      groups.push({ type: String.fromCharCode(97 + index), length: gap.length, count: 1, share: 0, color: GAP_COLORS[index] ?? '#9aa6b2' });
    }
  });
  groups.forEach((group) => { group.share = group.count / Math.max(1, gaps.length); });
  gaps.forEach((gap) => {
    const group = groups.find((item) => Math.abs(item.length - gap.length) < 1e-8);
    gap.type = group?.type ?? '?';
  });
  return groups;
}
...
function allocatorValues(scenario: Scenario): OrbitPoint[] {
  ...
  if (scenario.allocator === 'rotation') {
    for (let index = 0; index < scenario.pointCount; index += 1) {
      values.push(scenario.alphaMode === 'rational' ? rationalValue(scenario, index) : frac(scenario.phase + index * scenario.alpha));
    }
  } else if (scenario.allocator === 'random') {
    const random = makeRandom(scenario.seed);
    for (let index = 0; index < scenario.pointCount; index += 1) values.push(random());
  } else {
    ...
    for (let index = 1; index < scenario.pointCount; index += 1) {
      ...
      const currentGaps = current.map((value, i) => {
        ...
        return { start: value, length: i === current.length - 1 ? 1 - value + next : next - value };
      });
      const largest = [...currentGaps].sort((a, b) => b.length - a.length || a.start - b.start)[0];
...
function incrementalLineage(points: OrbitPoint[]): LineageStep[] {
  ...
  points.forEach((point, index) => {
    if (index === 0) {
      current.push(point);
      lineage.push({ index, value: point.value, parentId: null, parentLength: 1, childLengths: null, split: false });
      return;
    }
    const previous = gapsFromSorted(current);
    const parent = previous.find((gap) => {
      if (gap.start < gap.end) return point.value > gap.start + 1e-10 && point.value < gap.end - 1e-10;
      return point.value > gap.start + 1e-10 || point.value < gap.end - 1e-10;
    });
    const parentLength = parent?.length ?? 0;
    const first = parent
      ? (point.value >= parent.start ? point.value - parent.start : 1 - parent.start + point.value)
      : 0;
...
function fareySequence(order: number): Array<[number, number]> {
  ...
  for (let denominator = 1; denominator <= order; denominator += 1) {
    for (let numerator = 1; numerator < denominator; numerator += 1) {
      if (gcd(numerator, denominator) === 1) result.push([numerator, denominator]);
    }
  }
  result.push([1, 1]);
...
function fareyPrediction(alpha: number, pointCount: number): FareyPrediction {
  ...
  const [c, d] = right;
  const candidates = [
    { label: 'a', length: Math.max(0, b * alpha - a), count: Math.max(0, pointCount - b), color: GAP_COLORS[0] },
    { label: 'b', length: Math.max(0, c - d * alpha), count: Math.max(0, pointCount - d), color: GAP_COLORS[1] },
  ];
  const thirdCount = b + d - pointCount;
  if (thirdCount > 0) {
    candidates.push({ label: 'c', length: Math.max(0, (b - d) * alpha + c - a), count: thirdCount, color: GAP_COLORS[2] });
  }
  return { left, right, candidates, valid: candidates.every((candidate) => candidate.count > 0 && candidate.length > 0) };
}
...
function continuedFraction(value: number, depth = 9): ContinuedFraction {
  ...
  let pMinus2 = 0; let pMinus1 = 1; let qMinus2 = 1; let qMinus1 = 0;
  for (let index = 0; index < depth; index += 1) {
    ...
    pMinus2 = pMinus1; pMinus1 = p; qMinus2 = qMinus1; qMinus1 = q;
...
function dualResult(scenario: Scenario): DualResult {
  ...
  for (let index = 0; index < scenario.pointCount * 3; index += 1) {
    const value = scenario.alphaMode === 'rational'
      ? rationalValue(scenario, index)
      : frac(scenario.phase + index * scenario.alpha);
    if (scenario.strictBeta ? value < scenario.beta : value <= scenario.beta) selected.push(index);
  }
  const returnGaps = selected.slice(1).map((value, index) => value - selected[index]);
  const fakeGaps = returnGaps.map((length, index) => ({ id: 'return-' + index, start: index, end: index + length, length, type: '', rank: index }));
  ...
  return {
    ...
    groups,
    regime: groups.length === 0 ? 'empty' : groups.length === 2 ? 'two-gap' : groups.length === 3 ? 'three-gap' : 'boundary',
  };
}

function topology(gaps: Gap[], threshold: number): TopologyResult {
  const unique = [...new Set(gaps.map((gap) => Number(gap.length.toFixed(8))))].sort((a, b) => a - b);
  const events = unique.map((value) => ({
    threshold: value,
    components: Math.max(1, gaps.filter((gap) => gap.length > value + 1e-8).length),
    gapIds: gaps.filter((gap) => Math.abs(gap.length - value) < 1e-8).map((gap) => gap.id),
  }));
  const bars = gaps.map((gap, index) => ({ id: 'bar-' + index, birth: 0, death: gap.length }));
  return { threshold, components: Math.max(1, gaps.filter((gap) => gap.length > threshold + 1e-8).length), events, bars };
}

function word(gaps: Gap[], groups: GapGroup[]): WordResult {
  const palette = Object.fromEntries(groups.map((group) => [group.type, group.color]));
  ...
  return { word: gaps.map((gap) => gap.type).join(''), palette, symmetry: { anchor, matched, total: Math.max(0, Math.floor((gaps.length - 1) / 2)) } };
}
...
function lattice(alpha: number): LatticeVector[] {
  ...
  for (let x = -4; x <= 4; x += 1) {
    for (let y = -4; y <= 4; y += 1) {
      ...
      const projectedY = y;
      vectors.push({ x: projectedX, y: projectedY, norm: Math.hypot(projectedX, projectedY), active: false });
    }
  }
  const min = Math.min(...vectors.map((vector) => vector.norm));
  return vectors.map((vector) => ({ ...vector, active: vector.norm < min * 1.45 }));
}

function extension(alpha: number, beta: number, size: number): { points: number[]; distinctCount: number; groups: GapGroup[] } {
  const points: number[] = [];
  ...
  const gaps = sorted.map((value, index) => {
    const next = sorted[(index + 1) % sorted.length];
    return { id: 'ext-' + index, start: value, end: next, length: index === sorted.length - 1 ? 1 - value + next : next - value, type: '', rank: index };
  });
  const groups = gapGroups(gaps);
  return { points: sorted, distinctCount: groups.length, groups };
}

export function computeCertificate(input: Partial<Scenario>, topologyThreshold?: number): Certificate {
  ...
  const residual = lengths.length === 3 ? Math.abs(lengths[2] - lengths[1] - lengths[0]) : 0;
  const relation = lengths.length === 3
    ? lengths[2].toFixed(6) + ' = ' + lengths[1].toFixed(6) + ' + ' + lengths[0].toFixed(6)
    : 'not needed in a two-gap state';
  const sumCheck = { holds: lengths.length < 3 || residual < 1e-7, residual, relation };
  const contrast = scenario.allocator !== 'rotation';
  const collision = new Set(points.map((point) => point.value.toFixed(10))).size !== points.length;
  const theoremStatus = contrast ? 'contrast' : scenario.alphaMode === 'rational' || collision ? 'boundary' : sumCheck.holds ? 'certified' : 'warning';
  ...
  const threshold = topologyThreshold ?? Math.max(...gaps.map((gap) => gap.length)) * 0.55;
  return {
    scenario,
    points,
    sortedPoints,
    ...
    extension: extension(scenario.alpha, scenario.beta, Math.min(18, scenario.pointCount)),
  };
}

export function polar(value: number, radius: number, center: number): [number, number] {
